...
using namespace std;
//initialization
const int N=1, m=1, nsteps=1e3, npoints=1e3; //number of particles, mass, number of steps
const double de=10, alpha=2, dt=1e-3, dx=1e-3;//parameters of the Morse potential, time step
const double x0=0.01, p0=0, t0=0, f0=0, a0=0; //initialization
double etot, etot_num, t, f, v, x, a, a_1, x_1, tmp, f1, a1, ek, ep, et; //simulation parameters
double Pot_pos[npoints], Pot_SHO[npoints], Pot_real[npoints];//evaluate potential at each point
double Time[nsteps], Position[nsteps], Velocity[nsteps], Energy[nsteps], KinEnergy[nsteps], PotEnergy[nsteps];// simulation outputs
...
double omega, amp, phi; // analytic parameters for the corresponding Harmonic potential
double Pos[nsteps], Vel[nsteps], Ekin[nsteps], Epot[nsteps];// analytic outputs
int main(){
    //looks like both methods work, but don't agree: algebraic mistake?
    //Analytics {For the SHO approximation}
    omega=alpha*sqrt(2*de/m);//H-dependent, equate V''/2(x)^2 and m(omega*x)^2/2
    for (int i = 0; i < npoints; i++) {
        Pot_pos[i] = (i-nsteps/2)*dx;
        Pot_SHO[i]= (m*omega*omega*((i-nsteps/2)*dx)*((i-nsteps/2)*dx))/2; 
        Pot_real[i] = de*(1-exp(-alpha*((i-nsteps/2)*dx)))*(1-exp(-alpha*((i-nsteps/2)*dx)));
        //compute potential on the symmetric interval around 0
    }
    etot_num = de*(1-exp(-alpha*x0))*(1-exp(-alpha*x0))+p0*p0/2/m;
    etot = m*(omega*x0)*(omega*x0)/2 + p0*p0/2/m;
    amp = sqrt(2*etot/m)/omega;
    phi = acos(x0/amp);
    cout<<phi<<" "<<etot<<" "<<etot_num<<" "<<amp<<" "<<omega; 
    ...
    for(int i = 0; i < nsteps; i++) {
      ...
      KinEnergy[i] = m*v*v/2; PotEnergy[i] = de*(1-exp(-alpha*x))*(1-exp(-alpha*x)); 
      Energy[i] = KinEnergy[i]+PotEnergy[i];//energy
      a_1 = a; // not updated acceleration (from previous Time step)
      f = -2*alpha*de*(1-exp(-alpha*x))*exp(-alpha*x);//force=-dV/dx
      a = f/m;//acceleration
      t += dt;//increment time
    switch(method) {
        case 0://Euler
...
for (int i = 1; i<nsteps; i++){
  ...
  //Output
    ...
    for(int i = 0; i < nsteps; i++) {
 	outputfile<<Pot_pos[i]<<" "<<Pot_SHO[i]<<" "<<Pot_real[i]<<" "<<etot_num<<endl;
	}
    outputfile.close();//it's a good idea to always close the file once you are done
